...
from tree_t import get_dependancies, TreeT, Parse
import collections
import pickle
...
class Batcher(object):
    ...
    def _create_data(self, data_file):
        """ """
        data = {}
        for type, d_file in self._d_files.items():
            print('Processing {}'.format(d_file))
            tree_deps = get_dependancies(d_file)
            parse_list = []
            with open(d_file) as f:
                for line, dep in zip(f.readlines(), tree_deps):
                    parse_list.append(Parse(line, dep))
            data.setdefault(type,[]).extend(parse_list)
            print('Finished processing {}'.format(d_file))
    ...
    def _create_vocab(self):
        """ """
        _vocab = {}
        d_keys = list(vars(self._data['train'][0]).keys())
        d_keys.remove('gold')
        for k in d_keys:
            start_time = time.clock()
            v = [getattr(e, k) for d in self._data.values() for e in d ]
    ...
    def load_data(self):
        ...
        if not os.path.exists(data_file) or os.path.getsize(data_file) == 0:
            ...
            data = self._create_data(data_file)
        else:
        ...
        for k in data.keys():
            for d in data[k]:
                d.labels = [self._t_op.modify_tag(e) for e in d.labels]
        return data

    def get_batch(self, mode, shuffle=False):

        from itertools import islice, chain
        import random

        _step = int(np.ceil(1/self._precentage))
        data = islice(self._data[mode], 0, None, _step)

        if shuffle:
            data = list(data)
            random.shuffle(data)

        sourceiter = iter(data)
        while True:
            batchiter = islice(sourceiter, self._batch_size)
    ...
    def process(self, bv, add_unk=False):
        ...
        bv_w = self._vocab['words'].to_ids([b.words for b in bv])
        self._seq_len = self.seq_len(bv_w)
        seq_len_w, bv_w_in, max_len_w = self.process_words(bv_w, add_unk)
        # batch.update({'word': {'in': bv_w_in, 'len': seq_len_w}})
        batch.setdefault('word', {}).update({'in': bv_w_in, 'len': seq_len_w})

        bv_t = self._vocab['labels'].to_ids([b.labels for b in bv])
        seq_len_t, bv_t_in, bv_t_eos = self.process_tags(bv_t, max_len_w)
        # batch.update({'tag': {'in': bv_t_in, 'len': seq_len_t, 'out': bv_t_eos}})
        batch.setdefault('tag', {}).update({'in': bv_t_in, 'len': seq_len_t, 'out': bv_t_eos})

        bv_p = self._vocab['tags'].to_ids([b.tags for b in bv])
        bv_pos_in = self.process_pos(bv_p, max_len_w)
        # batch.update({'pos': {'in': bv_pos_in, 'out': bv_pos_in}})
        batch.setdefault('pos', {}).update({'in': bv_pos_in, 'out': bv_pos_in})

        bv_c = self._vocab['chars'].to_ids([b.chars for b in bv])
        seq_len_c, bv_c_in = self.process_chars(bv_c, max_len_w)
        # batch.update({'char': {'in': bv_c_in, 'len': seq_len_c}})
        batch.setdefault('char', {}).update({'in': bv_c_in, 'len': seq_len_c})

        bv_w_t = self.pad(bv_w, max(self._seq_len))
        bv_w_t_in = self._vocab['words'].to_tokens(bv_w_t)
        # batch.update({'word_t': {'in': bv_w_t_in, 'len': self._seq_len}})
        batch.setdefault('word_t', {}).update({'in': bv_w_t_in, 'len': self._seq_len})
        return batch
