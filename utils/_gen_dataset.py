...
class GenDataset():
    ...
    def generate_data_flat(src_dir, dest_dir = os.getcwd(), gen_tags_fn=gen_supertags):
    ...
    def generate_data(src_dir, dest_dir, gen_tags_fn=gen_supertags):
...
class superTag():
    ...
    def gen_range(tree, nid, minR, maxR):
        ...
        for child in tree.children(nid):
            ...
            (minR, maxR) = gen_range(tree, cid, minR, maxR)
    ...
    def gen_suptag(tree, nid, height_dict, path_dict):
        ...
        for child in tree.children(nid):
            cid = child.identifier
            leaf_id, height = gen_suptag(tree, cid, height_dict, path_dict)
